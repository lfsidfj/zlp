...
public class CustomOrderController extends BaseController {
	...
	String menuUrl = "customorder/list.do"; //菜单地址(权限用)
	...
	public ModelAndView list(Page page) throws Exception{
		...
		pd.put("ORDER_POSITION",2); // 定制商品订单
		page.setPd(pd);
		List<PageData>	varList = orderService.customList(page);	//列出Order列表
		mv.setViewName("zlp/order/customorder/order_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("zlp/order/customorder/order_edit");
	...
	public ModelAndView goInformation()throws Exception{
		...
		mv.setViewName("zlp/order/customorder/order_information");
	...
	public ModelAndView goLogistics()throws Exception{
		...
		mv.setViewName("zlp/order/customorder/order_logistics");
		mv.addObject("QX",Jurisdiction.getHC());	//按钮权限
		return mv;
	}
	/**去发货页面
	...
	public ModelAndView goShipments()throws Exception{
		...
		pd = orderService.findById(pd);	//根据ID读取
		mv.setViewName("zlp/order/customorder/order_shipments");
		mv.addObject("msg", "shipments");
		mv.addObject("pd", pd);
